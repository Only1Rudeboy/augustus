...
void platform_renderer_window_to_logical(int window_x, int window_y, float *logical_x, float *logical_y)
{
    ...
    *logical_y = (float) window_y;
    ...
    SDL_RenderWindowToLogical(data.renderer, window_x, window_y, logical_x, logical_y);
#else
    /* Fallback: manually undo display scale if logical size is active. */
    int scale = platform_screen_get_scale();
    if (scale > 0 && scale != 100) {
        *logical_x = (float) window_x * 100.0f / (float) scale;
        *logical_y = (float) window_y * 100.0f / (float) scale;
    }
#endif
}
